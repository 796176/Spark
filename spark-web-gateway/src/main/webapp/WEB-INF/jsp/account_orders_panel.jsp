...
<html lang="en-US">
	<head>
		...
		<link rel="stylesheet" href="/static/css/account_orders_panel.css"/>
	</head>
	<body>
		<header>
		...
		<main>
			<h1>
				<%= "Order Panel For Account " + accountId %>
			</h1>
			<% if (orders.length == 0) { %>
				<p>Empty</p>
			<% } else { %>
						...
						<li class="order decorated_li">
							<jsp:element name="a" class="decorated_embedded_link">
								<jsp:attribute name="href">
									<%= "/panel/account/" + accountId + "/order/" + order.orderId() %>
								</jsp:attribute>
								<jsp:body>
									<span class="order_id decorated_sli">
										<%= order.orderId() %>
									</span
									<span class="order_timestamp decorated_sli">
