...
public class RMQOrderEventConsumer implements Consumer {
	...
	private final OrderEventEnricher orderEventEnricher;

	private final Channel acknowledgementChannel;

	public RMQOrderEventConsumer(
		@Nonnull OrderEventEnricher orderEventEnricher,@Nonnull Channel acknowledgementChannel
	) {
		this.orderEventEnricher = orderEventEnricher;
	...
	public void handleDelivery(
		...
		try {
			orderEventEnricher.enrichEvent(eventType, contentType, version, bytes, messageId);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
