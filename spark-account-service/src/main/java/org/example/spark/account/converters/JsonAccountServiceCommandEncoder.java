...
package org.example.spark.account.converters;
...
public class JsonAccountServiceCommandEncoder implements AccountServiceCommandEncoder {
	...
	@Override
	public EncodedCommand encodeCreatingAccountCommand2(@Nonnull String name, @Nonnull String encodedPassword) {
	...
	@Override
	public EncodedCommand encodeCreatingAdministratorAccountCommand2(
		@Nonnull String name, @Nonnull String encodedPassword
	) {
		JsonFactory jsonFactory = new JsonFactory();
		ByteArrayOutputStream os = new ByteArrayOutputStream();
		try (JsonGenerator jsonGenerator = jsonFactory.createGenerator(ObjectWriteContext.empty(), os)) {
