...
import org.example.spark.account.models.EventEntity;
import org.example.spark.account.models.EventEntity_;
...
public class JPAPublishableEventDataAccess implements PublishableEventDataAccess {
	...
	@Override
	public PublishableAccountEvent[] retrieveInChronologicalOrder() {
		...
		return eventEntityList.stream().map(ee -> {
				return new PublishableAccountEvent(
					ee.getEventType(),
					ee.getContentType(),
					ee.getVersion(),
					ee.getId().toString(),
					ee.getEventBody().getBytes(StandardCharsets.UTF_8)
				);
			}
		).toArray(PublishableAccountEvent[]::new);
	}

	@Override
	public void delete(@Nonnull PublishableAccountEvent... events) {
		// DELETE FROM outbox WHERE id = events[0].getEventId() OR ...;
		CriteriaBuilder cb = entityManagerFactory.getCriteriaBuilder();
		...
		for (int i = 0; i < predicates.length; i++) {
			predicates[i] = cb.equal(eventEntity.get(EventEntity_.id), UUID.fromString(events[i].getEventId()));
		}
